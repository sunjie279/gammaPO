...
class SimPOTrainer(Trainer):
    ...
    def __init__(
        ...
        self.length_norm = args.length_norm
        ...
        self.gap_mean = 0   # also used for beta-SimPO
        self.gap_std = 1    # also used for beta-SimPO
    ...
    def get_gamma_b(self, 
                    reward_gap: torch.FloatTensor,
        ) -> torch.FloatTensor:
        # optimize the \gamma in one batch
        ...
        batch_size = reward_gaps.numel()
        p = torch.full_like(reward_gaps, fill_value=1/batch_size, requires_grad=True)
        for _ in range(20):
        ...
        gamma = gamma.detach().view(dist.get_world_size(), -1)
        if dist.get_rank()==0:
            print(f"[opt \gamma in one batch] tau={self.tau} | gamma={gamma} | reward_gaps={reward_gaps.view(-1)}")
        gamma = gamma[dist.get_rank(), :]
        return gamma
    ...
    def get_gamma_dq(self,
        ...
        print(f"[opt \gamma in queue] tau={self.tau} | gamma={gamma} | reward_gaps={reward_gaps.view(-1)[-reward_gap.numel():]}")
    ...
    def simpo_loss(
        ...
        if self.flip > 0:
            if random.random() <= self.flip:
                pi_logratios.neg_()  # In-place negation
        pi_logratios = pi_logratios.to(self.accelerator.device)
        reward_gap = self.beta * pi_logratios
        logits = pi_logratios - self.gamma_beta_ratio

        if self.loss_type == "simpo":
            print(f"[simpo{'-ls' if self.label_smoothing>0 else ''}] reward_gap={reward_gap}")
            losses = (
                -F.logsigmoid( reward_gap - self.gamma0) * (1 - self.label_smoothing)
                -F.logsigmoid(-reward_gap - self.gamma0) * self.label_smoothing
            )
        elif self.loss_type == "gm-simpo":
            logits = pi_logratios - self.get_gamma_dq(reward_gap) / self.beta
            losses = (
                -F.logsigmoid(self.beta * logits)
            )
        elif self.loss_type == "b-simpo":
            logits = pi_logratios - self.gamma_beta_ratio
            ...
            losses = (
                -F.logsigmoid(beta_used * logits)
            )
            # return gap_mean and gap_std for update them
            gap_mean, gap_std = reward_gaps.mean(), reward_gaps.std()
            if dist.get_rank()==0:
                print(f"beta-simpo | beta_used={beta_used.detach()} | gap_mean={gap_mean.detach()} | reward_gaps={reward_gaps.detach()}")
        elif self.loss_type == "cpo":
            losses = (
                -F.logsigmoid(self.beta * (policy_chosen_logps - policy_rejected_logps)) - self.gamma_beta_ratio * policy_chosen_logps
            )
        elif self.loss_type == "orpo":
            p_w = torch.exp(policy_chosen_logps)
            p_l = torch.exp(policy_rejected_logps)
            losses = (
                -torch.log(p_w) - self.gamma_beta_ratio * F.logsigmoid(torch.log(p_w/(1-p_w) * (1-p_l)/p_l))
            )
        else:
            raise ValueError(
                f"Unknown loss type: {self.loss_type}. Should be one of ['simpo', 'cpo', 'orpo', 'gm-simpo', 'b-simpo']"
            )

        chosen_rewards = self.beta * policy_chosen_logps.to(self.accelerator.device).detach()
        rejected_rewards = self.beta * policy_rejected_logps.to(self.accelerator.device).detach()

        if self.loss_type == "b-simpo":
    ...
    def get_batch_loss_metrics(
        ...
        losses, chosen_rewards, rejected_rewards, gap_mean, gap_std = self.simpo_loss(
            policy_chosen_logps,
            policy_rejected_logps,
        )
        if self.loss_type == "b-simpo":
